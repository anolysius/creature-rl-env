"""numpy <-> JAX parity for the opt-in super-effective-only economy (super-effective-economy).

``super_effective_only=True`` zeroes NEUTRAL *and* resisted (eff <= NEUTRAL) damage in both
engines. Scripted ceilings run on numpy ``CritterEnv``; learned agents run on the JAX env — the
variant is only trustworthy if both engines agree. This gate drives both from the same seed +
actions across {commit, non-commit} x {single-type, multi-type boss} with SE-only ON and asserts
every obs key, reward, terminated, truncated match (parity 0). Skipped when JAX is absent.
"""
...
def _envs(commit: bool, boss_secondary: bool):
    env = CritterEnv(
        ...
        min_gyms=_NGYM, boss_secondary=boss_secondary, super_effective_only=True,
    )
    jenv = make_jax_env(JaxEnvConfig(
        grid=_GRID, patch_radius=_PR, max_steps=_STEPS, max_gyms=_NGYM,
        commit=commit, super_effective_only=True,
    ))
    return env, jenv
...
def test_se_only_parity_gym_clearing(seed: int, commit: bool, boss_secondary: bool) -> None:
    assert _run_parity(seed, _gym_seeking_policy, commit=commit,
                       boss_secondary=boss_secondary) > 0


@pytest.mark.parametrize(("commit", "boss_secondary"), _CONFIGS)
def test_se_only_parity_random(commit: bool, boss_secondary: bool) -> None:
...
@pytest.mark.parametrize(("commit", "boss_secondary"), _CONFIGS)
def test_se_only_parity_heldout(commit: bool, boss_secondary: bool) -> None:
