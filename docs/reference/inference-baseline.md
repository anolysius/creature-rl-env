...
> ⚠️ **max_steps 의존성 (중요)**: `type_blind`(그리고 `probe`) floor 는 **에피소드 step 상한에
> 민감**하다 — 둘은 attrition(중립 무브)으로 버티므로, step 이 많을수록 중립 grind 무브가 쌓여
> SE-rate 비율이 *내려간다*. `oracle`/`infer` 는 안정적. 따라서 **band 는 LLM probe 와 *같은*
> max_steps 에서 읽어야 한다** — 러너는 `--telemetry` 에서 같은 sealed(같은 max_steps) 위에 band
> 를 자동 계산하므로 항상 사과-대-사과다. 아래 표는 두 운용점을 둘 다 싣는다:
> **`max_steps=40`**(러너 기본 = probe 운용점) / **`max_steps=200`**(SealedEvalSet 기본).

**demonstrator** (grid5, types3, boss 140/6/18) — 러너 inference-demo preset:

| arm | SE-rate n=8 (ms=40 / ms=200) | SE-rate n=16 (ms=40 / ms=200) | gym-clears | inference_score |
|---|---|---|---|---|
| oracle | **100% / 100%** | **100% / 100%** | 2.12 | 1.00 |
| infer | **90% / 90%** | **89% / 89%** | 2.12 | 1.00 |
| type_blind | **27% / 7%** | **13% / 3%** | 1.25 / 0.81 | 0.00 |
| probe | 0% / 0% | 0% / 0% | 0.00 | 0.00 |
...
|---|---|---|---|---|
| oracle | **100% / 100%** | **100% / 100%** | 2.12 | 1.00 |
| infer | **85% / 85%** | **85% / 85%** | 2.12 | 1.00 |
| type_blind | **20% / 4%** | **16% / 3%** | 1.00 / 0.94 | 0.00 |
| probe | 6% / 1% | 21% / 5% | 0.38 / 1.00 | 0.00–0.05 |

(`gym-clears`/`inference_score` 열은 max_steps=200 기준 — gym-clears 는 winnable gym 을 다 깨므로
step 상한에 둔감. 변별은 SE-rate 로 읽는다.)
...
- **SE-rate 가 attrition-proof 변별자다**: oracle 100% > infer 85–90% ≫ type_blind(ms=40 ~20–27% /
  ms=200 ~3–7%) > probe 0%. 추론 행위(=super-effective 무브 선택)를 승리/소모전과 분리해 직접 잰다.
  **LLM 은 *같은 max_steps* 의 이 band 위 어디에 떨어지는가로 읽어라**(러너가 자동으로 같은 운용점에
  band 를 계산).
...
python scripts/llm_eval_run.py --provider claude-cli \
  ...
  --max-steps 40 --battle-memory --telemetry --runs 3
# 또는 API
python scripts/llm_eval_run.py --provider anthropic --model claude-opus-4-8 [...동일 노브...]
```

- `--max-steps` 를 **명시**하라 — band 의 type_blind/probe floor 가 step 상한에 민감(위 ⚠️).
  러너는 submission 과 band 를 *같은* max_steps 로 계산하므로 출력은 항상 사과-대-사과.
