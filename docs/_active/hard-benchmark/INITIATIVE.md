...
(이후: SOTA-class agent, family 확장, 상대-깊이 질문 재도전(더 무는 config — **새 scout 선행 조건**). recurrence axis(#1·#2)·메모리 agent 절대 난이도(#3)·다중타입 env+scout(#4)·다중타입 헤드룸 측정(#5, B=inconclusive 정직 종결) settled.)
