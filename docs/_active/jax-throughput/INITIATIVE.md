...
| 13 | `gpu-ec3-result` | ✅ done (→ `_archive/2026-Q2/jax-throughput/13-gpu-ec3-result/`) | **M4-EC3 GPU 실측 기록 + bench 배치 상한 정직 수정** — #12 Colab bench를 사용자가 무료 **NVIDIA T4**서 실행 → 실측 확보. overworld fused-scan vmap **75.9M(b1024)→271M(b4096)→952.8M(b16384) steps/s** = **≥10M EC의 95×, batch 단조 스케일** → **M4-EC3 달성**(milestones M4 EC1·EC2·EC3 ✅ 갱신; doc lag 정직화). `gpu_bench.py` 기본 batch 상한 **65536→16384**(b65536이 무료 T4 멈춤·full-episode OOM → 기본 제외; 로직·시그니처 무변경, `--quick` 무영향). **정직 경계(과대 금지)**: overworld slice 한정·single run·free T4; full-episode GPU 수치는 분기 많은 step의 free-T4 컴파일 한계로 미확보(단 **CPU 22M/s로 EC 이미 초과** → GPU도 초과 자명, 정밀치는 better-HW minor 후속). competitive-analysis "Speed" 행 ✅(JAX vmap ~950M GPU, 채택-게이트 갭 해소) + jax-throughput.md §5 item3(미측정→실측) 갱신. src 무변경 → 442 무회귀, mypy(28)/ruff/build clean. _(commit pending)_ |
