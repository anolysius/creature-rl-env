...
| # | slug | 상태 | 한 줄 |
|---|---|---|---|
| 14 | `render-obs-tile-codes` | ✅ done (→ `_archive/2026-Q2/eval-product/14-render-obs-tile-codes/`) | **render_obs 타일-코드 버그 — LLM 측정 floor의 root-cause 후보 수정** — `render_obs`(LLM 텍스트 맵)가 gym=code 3·creature=code 2를 가정했으나 env는 `_PATCH_CREATURE=1,_PATCH_GYM=2`를 emit → LLM이 **생물을 "#"벽으로, 체육관을 "C"생물로** 보고 "G"는 안 뜸. scripted는 raw obs라 정상, **LLM만** 뒤섞인 지도(=#6·#7 증상). 미검출 원인: 합성 테스트가 render_obs의 *틀린 가정*을 그대로 씀. 수정: env `_PATCH_*` 상수 import해 글리프·살라언스·center 분기에 사용(SSOT 드리프트 방지) + 신규 테스트 2(상수 대조 + 실 env, `"#" not in grid`). 무회귀: render_obs는 scripted 채점 경로 미참조 → byte-identical. 502→**504**(+2, 회귀0), mypy/ruff clean. L3 APPROVE. **정직 경계**: 정합 수정이며 "floor가 풀린다"는 재측정 후속 가설(증명 아님). 후속: merge 후 재측정 / `baselines.py` SSOT hygiene. |
