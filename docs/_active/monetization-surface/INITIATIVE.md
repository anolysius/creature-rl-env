...
| # | slug | 상태 | 한 줄 |
|---|---|---|---|
| 10 | `community-leaderboard` | ✅ done (→ `_archive/2026-Q2/monetization-surface/10-community-leaderboard/`) | **커뮤니티 리더보드 (M5-EC3 artifact, 사용자 지시)** — 시즌제 공개 시험지(`season_seeds` 공개 유도, sealed·시즌·default 분리 테스트 고정) + `self_reported:true` 강제 스키마 + validate(CI 게이트)/load(랭킹) + `--demo` 실측 예시(0.750) + 사이트 시즌별 표(정직 라벨 3종·빈-상태·escape) + 가이드 en/ko. 지표 비혼동 명시. 20 테스트, 630→**650**, L3 2/2. 사람 게이트: 접수 공지·Pages·시즌 개시·Hub. |
| 9 | `hard-note-precision` | ✅ done (→ `_archive/2026-Q2/monetization-surface/09-hard-note-precision/`) | **hard note 정밀화** — "recurrent OPEN"이 과소 서술이 됨(#3/#5 가 관련 config 에서 rec ~32–43% 측정) → 3단 구분(ff 유지/related 측정/exact+SOTA=OPEN) 정밀화. SSOT 파급 전부 갱신(문서·데모·site). 추가 stale 수리(데모의 낡은 sealed-드롭 서술, #7 이후). 630 passed, L3 2/2. |
| 8 | `site-tier-section` | ✅ done (→ `_archive/2026-Q2/monetization-surface/08-site-tier-section/`) | **사이트 난이도-티어 섹션 (M5-EC2/EC3 연결)** — 판매-표면(#5·#6)을 런치 사이트에 노출. "Difficulty tiers" 섹션을 `env_tier` **SSOT 원문 렌더**(하드코딩 0, 사이트가 코드보다 세게 주장 불가). L1 BLOCK 반영=`_SITE_TIERS` 고정 리스트(테스트-등록 custom 티어 누출 차단)+실증 테스트. L3 SUGGEST 반영="프로토타입 buyer flow" 한정어. 양 언어+브라우저 확인. 4 테스트, 626→**630**, L3 2/2 APPROVE. 공개 배포=사람 게이트. |
