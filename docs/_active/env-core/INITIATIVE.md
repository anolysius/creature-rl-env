...
- ✅ **(i) 용량+예산 동시 스케일 완료(task 30) — PARTIAL**: lever는 **예산**(150k 2.07→250k 2.44, 2.5 근접)이지
  **용량 아님**(big-net robust 하락). **#28 "compute 병목 아님"을 정정**(저예산 외삽). baseline@250k서 held-in 2.44·
  gap −0.05≈0 = 가장 긍정적이나 단일 fold/config·held-in<2.5=주장 아님. **경계 미종결 — 예산 cheap-ish 아직 오름.**
- **다음(가장 직접적)** — **더 큰 예산**(예: 400~500k)으로 baseline-net held-in이 **2.5를 robust하게 넘는지** +
  넘으면 **full-LOO confound-reduced gap 재측정**(AC3 보류분). 수확체감이면 (B)=“예산으로 부분 회복, 완전 회복엔
  표현/메타-RL 필요”로 패키징. (용량 스케일은 배제됨.)
- 이후 *비싼/다른* 경로(예산 plateau 시): 메커닉-범용 표현(family/task embedding=contextual MDP) / 커리큘럼 /
  메타-RL few-shot. 또는 **(B)를 "정직한 열린 문제"로 패키징하고 난이도·JAX로 피벗**.
