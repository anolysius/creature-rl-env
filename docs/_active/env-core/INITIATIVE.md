...
| 32 | `duel-fewshot-adapt` | ✅ done (→ `_archive/2026-Q2/env-core/33-duel-fewshot-adapt/`) | **(B) duel 제로샷 불가 *메커니즘 입증* + few-shot SLOW — (B) 스레드 종결**: #32 frontier(duel) 공략. `charge_trace`+`test_charge_degenerate_in_train_families`(numpy-only): train(critter/forage/muster) charge≡0/duel battle>0 → **degenerate feature → 제로샷 원리 불가** 입증(일반 명제: 새 메커닉이 train서 degenerate한 obs 의존 시 제로샷 불가). `fewshot_adapt_curve`+`--fewshot`(held-out duel fine-tune, eval seed≠adapt seed). **실측**(base 150k, 5run): zero-shot 0.65±0.20/25k 0.59/50k 0.76/**100k 1.45±0.48** → **verdict=SLOW**(≤50k 노이즈, 100k에야 ~2배). 단일seed pilot 50k≈1.6→**multi-run 4번째 교정**. duel은 적응으로 닿지만 *느림*(~100k=base 2/3)=거의 새 skill. **(B) 종결: 메커닉 이웃 제로샷 전이/구조적 새 배틀시스템(duel) 제로샷 원리불가+느린 few-shot = sharply characterized partial result.** env 코어 무변경. 197→199(+2, 회귀 0), mypy(22)/ruff/build clean. DESIGN §3.1.1+genre-generalization.md 갱신 |
...
- ✅ **(B) 스레드 종결(task 30~32)**: 예산 RECOVERY(held-in 2.75)·용량 배제 → 회복 상태 full-LOO에서 메커닉 이웃
  (critter/forage/muster) 전이 OK·**duel robust 실패**, 그 *원인*을 charge degenerate로 입증(제로샷 원리 불가),
  few-shot은 SLOW(~100k). **(B) 최종 = sharply characterized partial result**: 메커닉 이웃 제로샷 전이 / 구조적 새
  배틀시스템(duel) 제로샷 원리불가 + 느린 few-shot. 전부 박제: [`genre-generalization.md`](../../explanation/genre-generalization.md).
- **다음(둘 중 택1, 사람 결정)** — **(A) (B) 더 깊이**: 메커닉-범용 표현/메타-RL로 duel few-shot을 *빠르게*(현재 SLOW
  ~100k 단축) — 새 아키텍처 = 큰 작업. / **(B') 피벗·패키징**: (B)를 "structurally characterized partial result"로
  arXiv 패키징하고 갭 register의 **난이도 스케일 / JAX 속도**(공개 전 신뢰성·채택 게이트)로 전환 — **권장**(제품 leverage 큼).
