...
- **다음(신호→주장의 *유일하게 남은* 직접 경로)** — **(a') 절대 skill 향상 = 정책/obs 표현 개선**: 메커닉-범용
  obs 인코딩 / 더 큰 net / 보상·커리큘럼으로 held-in을 #26 수준(≈2.9)까지 끌어올린 뒤 gap 재측정해야
  generalist-mediocrity confound가 비로소 제거되고 (B)가 주장으로 이동. (compute·seed로는 한계 입증됨 — task 28.)
- 이후 옵션: (c) 5~6번째 family / JAX 포트(속도=채택 게이트) / 난이도 ladder. **공개(OSS/arXiv)는 맨 마지막**(사람 게이트).
