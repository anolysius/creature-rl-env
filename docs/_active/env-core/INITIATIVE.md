...
- **다음(남은 *비싼* 경로만)** — (B)를 주장으로 옮기려면: (i) **용량+예산 동시 스케일**(이번엔 net만↑·예산 50k=underfit),
  (ii) 메커닉-범용 표현(family/task embedding = contextual MDP), (iii) 커리큘럼, (iv) 메타-RL few-shot 적응.
  또는 **(v) 현 결과로 (B)를 "정직한 열린 문제"로 패키징하고 다른 갭(난이도·JAX)으로 피벗** — 제품 신뢰성엔 이게 더 값일 수도.
